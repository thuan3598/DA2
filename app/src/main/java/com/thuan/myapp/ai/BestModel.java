...
public final class BestModel {
  @NonNull
  private final Model model;

  private BestModel(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "best_model.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static BestModel newInstance(@NonNull Context context) throws IOException {
    return new BestModel(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static BestModel newInstance(@NonNull Context context, @NonNull Model.Options options)
      throws IOException {
    return new BestModel(context, options);
  }
